...
-- Only set for those municipalities where there is sewer information about
SET @getgmcode = CURSOR FOR
SELECT distinct [GMCODE]
  FROM [VWSSTATIC].[SAFETY_REGIONS_PER_MUNICIPAL]
...
BEGIN
    ...
    EXEC [dbo].[SafeInsertProtoConfiguration] @gmcode,  'sewer_measurements', 'VWSDEST.V_SEWER_MEASUREMENTS_PER_MUNICIPALITY', 'true', 'GMCODE', @gmcode, 'false', NULL, NULL, 'true'
    FETCH NEXT
    FROM @getgmcode INTO @gmcode
END
