...
BEGIN
    INSERT INTO VWSDEST.ELDERLY_AT_HOME
        ...
        SELECT
            ...
            SUM(TOTAL_CASES_REPORTED) AS INFECTED_ELDERLY_AT_HOME_DAILY,
			--calculate the infected inhabitants in the region per 100k
            CAST((CAST((SUM(TOTAL_CASES_REPORTED)) as decimal(10,2))/ SUM(INHABITANTS)*100000) as decimal(10,2)) AS INFECTED_ELDERLY_AT_HOME_DAILY_PER_100K,
            SUM(TOTAL_DECEASED_REPORTED) AS DECEASED_ELDERLY_AT_HOME_DAILY
		...
		--group by in order to sum over all security regions
        GROUP BY DATE_LAST_INSERTED, DATE_OF_STATISTIC_REPORTED
