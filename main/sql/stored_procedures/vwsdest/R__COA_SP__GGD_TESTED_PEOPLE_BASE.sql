...
CREATE OR ALTER PROCEDURE DBO.SP_GGD_TESTED_PEOPLE_BASE
AS
BEGIN
...
SELECT  
      ...
      ,ROUND(AVG(CAST([TESTED_WITH_RESULT]  AS FLOAT)) OVER (PARTITION BY SECURITY_REGION_CODE ORDER BY [DATE_OF_STATISTICS] ASC ROWS BETWEEN 6 PRECEDING AND CURRENT ROW),0) as [7D_AVERAGE_TESTS]
      ,ROUND(AVG(CAST([TESTED_POSITIVE]     AS FLOAT)) OVER (PARTITION BY SECURITY_REGION_CODE ORDER BY [DATE_OF_STATISTICS] ASC ROWS BETWEEN 6 PRECEDING AND CURRENT ROW),0) as [7D_AVERAGE_POSITIVE_TESTS]
      ,ROUND(AVG(CAST([TESTED_WITH_RESULT]  AS FLOAT)) OVER (PARTITION BY SECURITY_REGION_CODE ORDER BY [DATE_OF_STATISTICS] ASC ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING),0) as [7D_AVERAGE_TESTS_LAG]
      ,ROUND(AVG(CAST([TESTED_POSITIVE]     AS FLOAT)) OVER (PARTITION BY SECURITY_REGION_CODE ORDER BY [DATE_OF_STATISTICS] ASC ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING),0) as [7D_AVERAGE_POSITIVE_TESTS_LAG]
FROM BASE_CTE
)
...
SELECT
      ...
      ,dbo.CONVERT_DATETIME_TO_UNIX([DATE_OF_STATISTICS_LAG]) AS  OLD_DATE_OF_REPORT_UNIX
      ,[SECURITY_REGION_CODE]
      ,[SECURITY_REGION_NAME]
      ,TESTS
      ,POSITIVE_TESTS
      ...
      ,[7D_AVERAGE_TESTS]
      ,[7D_AVERAGE_TESTS_LAG]
      ,ROUND([7D_AVERAGE_TESTS] - [7D_AVERAGE_TESTS_LAG], 1)                              AS [7D_AVERAGE_TESTS_DIFF]
      ,[7D_AVERAGE_POSITIVE_TESTS]
      ,[7D_AVERAGE_POSITIVE_TESTS_LAG]
      ,ROUND([7D_AVERAGE_POSITIVE_TESTS] - [7D_AVERAGE_POSITIVE_TESTS_LAG]         ,0)    AS [7D_AVERAGE_POSITIVE_TESTS_DIFF]
      ,ROUND([7D_AVERAGE_POSITIVE_TESTS]*100     / NULLIF([7D_AVERAGE_TESTS],0)    ,1)    AS [7D_AVERAGE_PERCENTAGE_POSITIVE]
      ,ROUND([7D_AVERAGE_POSITIVE_TESTS_LAG]*100 / NULLIF([7D_AVERAGE_TESTS_LAG],0),1)    AS [7D_AVERAGE_PERCENTAGE_POSITIVE_LAG]
      ,ROUND((ROUND([7D_AVERAGE_POSITIVE_TESTS]*100     / NULLIF([7D_AVERAGE_TESTS],0),1) - ROUND([7D_AVERAGE_POSITIVE_TESTS_LAG]*100     / NULLIF([7D_AVERAGE_TESTS_LAG],0),1)), 1)  AS [7D_AVERAGE_PERCENTAGE_POSITIVE_DIFF]
FROM TOTALS_CTE
