...
CREATE OR ALTER PROCEDURE [dbo].[SP_POSITIVE_TESTED_PEOPLE_PER_MUNICIPALITY_ARCHIVE_DEST]
AS
BEGIN
    ...
    WHERE DATE_LAST_INSERTED IN (
        ...
        FROM (
            ...
            FROM [VWSDEST].[POSITIVE_TESTED_PEOPLE_PER_MUNICIPALITY]) SubQueryA
        ) SubQueryB
        WHERE SubQueryB.RANKING > 2)SubQueryC)) 

    INSERT INTO VWSARCHIVE.POSITIVE_TESTED_PEOPLE_PER_MUNICIPALITY
        ([DATE_OF_REPORT]
      ,[DATE_OF_REPORT_UNIX]
      ,[MUNICIPALITY_CODE]
      ,[MUNICIPALITY_NAME]
    ...
    DELETE FROM VWSDEST.POSITIVE_TESTED_PEOPLE_PER_MUNICIPALITY
    WHERE DATE_LAST_INSERTED IN (
        SELECT SubQueryC.DATE_LAST_INSERTED
        FROM(
        SELECT SubQueryB.DATE_LAST_INSERTED, SubQueryB.RANKING 
