...
CREATE OR ALTER PROCEDURE dbo.SP_RIVM_COVID_19_MUNICIPALITY_CUMULATIVE_ARCHIVE_INTER
AS
BEGIN
    ...
    FROM VWSINTER.RIVM_COVID_19_NUMBER_MUNICIPALITY_CUMULATIVE
     WHERE DATE_LAST_INSERTED IN (
        ...
        FROM (
            ...
            FROM VWSINTER.RIVM_COVID_19_NUMBER_MUNICIPALITY_CUMULATIVE) SubQueryA
        ) SubQueryB
        WHERE SubQueryB.RANKING > 2)SubQueryC))


    INSERT INTO VWSARCHIVE.RIVM_COVID_19_NUMBER_MUNICIPALITY_CUMULATIVE_ARCHIVE_INTER
        (DATE_OF_REPORT,
        MUNICIPALITY_CODE,
        MUNICIPALITY_NAME,
        PROVINCE,
    ...
    DELETE FROM VWSINTER.RIVM_COVID_19_NUMBER_MUNICIPALITY_CUMULATIVE
     WHERE DATE_LAST_INSERTED IN (
        SELECT SubQueryC.DATE_LAST_INSERTED
        FROM(
        SELECT SubQueryB.DATE_LAST_INSERTED, SubQueryB.RANKING
