...
BEGIN
    ...
    -- allow empty values and make them null
        IIF(TRIM([IC_BEDDEN_COVID])    ='',NULL,TRIM([IC_BEDDEN_COVID])    ) ,
        IIF(TRIM([IC_BEDDEN_NON_COVID])='',NULL,TRIM([IC_BEDDEN_NON_COVID])) ,
        IIF(TRIM([KLINIEK_BEDDEN])     ='',NULL,TRIM([KLINIEK_BEDDEN])     ) 
    FROM
