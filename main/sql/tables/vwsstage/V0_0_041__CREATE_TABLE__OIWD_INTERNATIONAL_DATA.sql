...
IF NOT EXISTS(SELECT * FROM sys.sequences WHERE object_id = OBJECT_ID(N'[dbo].[SEQ_VWSSTAGE_OIWD_International_Data]') AND type = 'SO')
CREATE SEQUENCE SEQ_VWSSTAGE_OIWD_International_Data
  START WITH 1
  INCREMENT BY 1;
GO
...
CREATE TABLE VWSSTAGE.OIWD_International_Data(
  [ID] INT PRIMARY KEY NOT NULL DEFAULT (NEXT VALUE FOR [DBO].[SEQ_VWSSTAGE_OIWD_International_Data]),
...
IF NOT EXISTS(SELECT * FROM sys.indexes WHERE NAME='IX_VWS_VWSSTAGE_OIWD_International_Data')
    CREATE NONCLUSTERED INDEX IX_VWS_VWSSTAGE_OIWD_International_Data
    ON VWSSTAGE.OIWD_International_Data(DATE_LAST_INSERTED)
