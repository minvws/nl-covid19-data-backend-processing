...
IF NOT EXISTS(SELECT * FROM sys.sequences WHERE object_id = OBJECT_ID(N'[dbo].[SEQ_VWSDEST_NICE_HOSPITAL_NL]') AND type = 'SO')
CREATE SEQUENCE SEQ_VWSDEST_NICE_HOSPITAL_NL
  START WITH 1
  INCREMENT BY 1;
GO

CREATE TABLE VWSDEST.NICE_HOSPITAL_NL(
	[ID] INT PRIMARY KEY NOT NULL DEFAULT (NEXT VALUE FOR [dbo].[SEQ_VWSDEST_NICE_HOSPITAL_NL]),
...
ON VWSDEST.NICE_HOSPITAL_NL(DATE_OF_STATISTICS, DATE_LAST_INSERTED)
