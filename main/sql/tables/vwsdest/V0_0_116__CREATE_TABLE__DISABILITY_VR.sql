...
IF NOT EXISTS(SELECT * FROM sys.sequences WHERE object_id = OBJECT_ID(N'[dbo].[SEQ_VWSDEST_DISABILITY_VR]') AND type = 'SO')
CREATE SEQUENCE SEQ_VWSDEST_DISABILITY_VR
  START WITH 1
  INCREMENT BY 1;
GO

CREATE TABLE VWSDEST.DISABILITY_VR(
  [ID] INT PRIMARY KEY NOT NULL DEFAULT (NEXT VALUE FOR [dbo].[SEQ_VWSDEST_DISABILITY_VR]),
