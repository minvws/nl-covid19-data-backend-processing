...
IF NOT EXISTS(SELECT * FROM sys.sequences WHERE object_id = OBJECT_ID(N'[dbo].[SEQ_VWSDEST_GGD_TESTED_PEOPLE_VR]') AND type = 'SO')
CREATE SEQUENCE SEQ_VWSDEST_GGD_TESTED_PEOPLE_VR
  START WITH 1
  INCREMENT BY 1;
GO

CREATE TABLE VWSDEST.GGD_TESTED_PEOPLE_VR(
       [ID] INT PRIMARY KEY NOT NULL DEFAULT (NEXT VALUE FOR [dbo].[SEQ_VWSDEST_GGD_TESTED_PEOPLE_VR])
...
ON VWSDEST.GGD_TESTED_PEOPLE_VR([DATE_OF_STATISTICS], DATE_OF_STATISTICS_LAG, DATE_LAST_INSERTED, REGION_CODE)
INCLUDE (
       DATE_RANGE_START
