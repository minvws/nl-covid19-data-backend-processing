...
CREATE  OR ALTER  VIEW [VWSDEST].[V_DIFFERENCE_INFECTIOUS_PEOPLE_COUNT_NORMALIZED__INFECTIOUS_AVG_NORMALIZED] AS
...
SELECT T1.[DATE_OF_REPORT]
      ...
      ,CASE WHEN T1.[INFECTIOUS_AVG_NORMALIZED] IS NULL THEN 0 ELSE T1.[INFECTIOUS_AVG_NORMALIZED] END AS [NEW_VALUE]
	  ,CASE WHEN T2.[INFECTIOUS_AVG_NORMALIZED] IS NULL THEN 0 ELSE T2.[INFECTIOUS_AVG_NORMALIZED] END AS [OLD_VALUE]
	  ,CASE WHEN (T1.[INFECTIOUS_AVG_NORMALIZED] - T2.[INFECTIOUS_AVG_NORMALIZED]) IS NULL THEN 0 ELSE (T1.[INFECTIOUS_AVG_NORMALIZED] - T2.[INFECTIOUS_AVG_NORMALIZED]) END AS [DIFFERENCE]
      ,T1.[DATE_LAST_INSERTED]
	  ,T2.[DATE_LAST_INSERTED] AS [PREVIOUS_DATE_LAST_INSERTED]
  FROM [VWSDEST].[INFECTIOUS_PEOPLE] T1
  LEFT JOIN (SELECT DATE_OF_REPORT, DATE_OF_REPORT_UNIX, DATE_LAST_INSERTED, INFECTIOUS_AVG_NORMALIZED
				FROM [VWSDEST].[INFECTIOUS_PEOPLE] 
