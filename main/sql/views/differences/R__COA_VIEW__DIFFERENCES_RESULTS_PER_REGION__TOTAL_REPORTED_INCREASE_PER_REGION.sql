...
CREATE OR ALTER  VIEW [VWSDEST].[V_DIFFERENCE_RESULTS_PER_REGION__TOTAL_REPORTED_INCREASE_PER_REGION] AS
...
SELECT 
	...
	LAG(DBO.NO_NEGATIVE_NUMBER_F(TOTAL_REPORTED_INCREASE_PER_REGION), 1, NULL) OVER (PARTITION BY [VRCODE] ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [OLD_VALUE],
	DBO.NO_NEGATIVE_NUMBER_F(TOTAL_REPORTED_INCREASE_PER_REGION) -
		LAG(DBO.NO_NEGATIVE_NUMBER_F(TOTAL_REPORTED_INCREASE_PER_REGION), 1, NULL) OVER (PARTITION BY [VRCODE] ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [DIFFERENCE]
...
SELECT DATE_OF_REPORT
	  ...
	  ,CASE WHEN OLD_VALUE IS NULL THEN 0 ELSE DBO.NO_NEGATIVE_NUMBER_I(OLD_VALUE) END AS OLD_VALUE
	  ,CASE WHEN [DIFFERENCE] IS NULL THEN 0 ELSE CONVERT(INTEGER, [DIFFERENCE]) END AS [DIFFERENCE]
FROM BASE_CTE T1
