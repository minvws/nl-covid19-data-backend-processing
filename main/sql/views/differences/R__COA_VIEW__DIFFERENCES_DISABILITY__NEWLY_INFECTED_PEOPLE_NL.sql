...
/****** Object:  View [VWSDEST].[V_DIFFERENCES_DISABILITY__NEWLY_INFECTED_PEOPLE_NL]    Script Date: 18-01-2021 ******/
CREATE OR ALTER  VIEW [VWSDEST].[V_DIFFERENCES_DISABILITY__NEWLY_INFECTED_PEOPLE_NL] AS
...
WITH LAST_DATE_OF_REPORT AS (
	...
	WHERE DATE_OF_REPORT >=  '2020-02-27 00:00:00.000' --In line with disability views
        AND DATE_OF_REPORT <  (SELECT MAX(DATE_OF_REPORT) FROM VWSDEST.DISABILITY) --In line with diability views. Neglect last value
...
SELECT 
	...
	LAG(DBO.NO_NEGATIVE_NUMBER_I([INFECTED_DISABILITY_DAILY]), 1, NULL) OVER (ORDER BY DATE_LAST_INSERTED,[DATE_OF_REPORT_UNIX]) AS [OLD_VALUE],
	DBO.NO_NEGATIVE_NUMBER_I([INFECTED_DISABILITY_DAILY]) -
		LAG(DBO.NO_NEGATIVE_NUMBER_I([INFECTED_DISABILITY_DAILY]), 1, NULL) OVER (ORDER BY DATE_LAST_INSERTED,[DATE_OF_REPORT_UNIX]) AS [DIFFERENCE]
FROM VWSDEST.DISABILITY
WHERE DATE_OF_REPORT >=  '2020-02-27 00:00:00.000' --In line with disability views
