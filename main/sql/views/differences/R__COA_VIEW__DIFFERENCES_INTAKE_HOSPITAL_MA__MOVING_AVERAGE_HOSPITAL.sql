...
/****** Object:  View [VWSDEST].[V_DIFFERENCE_INTAKE_HOSPITAL_MA__MOVING_AVERAGE_HOSPITAL]    Script Date: 12-11-2020 14:54:38 ******/

CREATE OR ALTER VIEW [VWSDEST].[V_DIFFERENCE_INTAKE_HOSPITAL_MA__MOVING_AVERAGE_HOSPITAL] AS
WITH LAST_DATE_OF_REPORT AS (
	SELECT
		CAST(MAX(DATE_OF_REPORT) as datetime) AS [LAST_DATE_OF_REPORT]
	FROM VWSDEST.HOSPITAL_ADMISSIONS_NICE
	WHERE DATE_OF_REPORT >=  '2020-03-16 00:00:00.000'
		AND DATE_LAST_INSERTED = (SELECT MAX(DATE_LAST_INSERTED) FROM VWSDEST.HOSPITAL_ADMISSIONS_NICE)
),
...
SELECT 
    ...
    LAG(DBO.NO_NEGATIVE_NUMBER_F([MOVING_AVERAGE_HOSPITAL]), 1, NULL) OVER (ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [OLD_VALUE],
	DBO.NO_NEGATIVE_NUMBER_F([MOVING_AVERAGE_HOSPITAL]) -
		LAG(DBO.NO_NEGATIVE_NUMBER_F([MOVING_AVERAGE_HOSPITAL]), 1, NULL) OVER (ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [DIFFERENCE]
