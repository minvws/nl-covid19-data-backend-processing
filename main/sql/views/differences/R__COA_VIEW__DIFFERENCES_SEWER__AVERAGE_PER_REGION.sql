...
/****** Object:  View [VWSDEST].[V_DIFFERENCE_SEWER__AVERAGE_PER_REGION]    Script Date: 12-11-2020 15:14:55 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER VIEW [VWSDEST].[V_DIFFERENCE_SEWER__AVERAGE_PER_REGION] AS
...
SELECT 
	WEEK_UNIX,
	VRCODE,
	LAG([WEEK_UNIX], 1, NULL) OVER (PARTITION BY [VRCODE] ORDER BY [WEEK_UNIX]) AS [OLD_DATE_OF_REPORT_UNIX],
    LAG(AVERAGE, 1, NULL) OVER (PARTITION BY [VRCODE] ORDER BY [WEEK_UNIX]) AS [OLD_VALUE],
	AVERAGE -
		LAG(AVERAGE, 1, NULL) OVER (PARTITION BY [VRCODE] ORDER BY [WEEK_UNIX]) AS [DIFFERENCE]
--  dbo.CONVERT_DATETIME_TO_UNIX(DATE_LAST_INSERTED) AS DATE_OF_INSERTION_UNIX
FROM PREPARATION
)
