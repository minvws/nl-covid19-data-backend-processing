...
/****** Object:  View [VWSDEST].[V_DIFFERENCE_NURSING_HOME__INFECTED_LOCATIONS_TOTAL_PER_REGION]    Script Date: 12-11-2020 15:14:55 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER VIEW [VWSDEST].[V_DIFFERENCE_NURSING_HOME__INFECTED_LOCATIONS_TOTAL_PER_REGION] AS
...
SELECT 
    ...
    LAG(DBO.NO_NEGATIVE_NUMBER_I(TOTAL_REPORTED_LOCATIONS), 1, NULL) OVER (PARTITION BY [VRCODE] ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [OLD_VALUE],
	DBO.NO_NEGATIVE_NUMBER_I(TOTAL_REPORTED_LOCATIONS) -
		LAG(DBO.NO_NEGATIVE_NUMBER_I(TOTAL_REPORTED_LOCATIONS), 1, NULL) OVER (PARTITION BY [VRCODE] ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [DIFFERENCE]
