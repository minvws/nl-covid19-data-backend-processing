...
SELECT 
	...
	LAG(DBO.NO_NEGATIVE_NUMBER_I([INFECTED_DAILY_TOTAL]), 1, NULL) OVER (ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [OLD_VALUE],
	DBO.NO_NEGATIVE_NUMBER_I([INFECTED_DAILY_TOTAL]) -
		LAG(DBO.NO_NEGATIVE_NUMBER_I([INFECTED_DAILY_TOTAL]), 1, NULL) OVER (ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [DIFFERENCE]
