...
/****** Object:  View [VWSDEST].[V_DIFFERENCE_RESULTS_PER_REGION__HOSPITAL_TOTAL_COUNTS_PER_REGION]    Script Date: 12-11-2020 15:14:55 ******/

CREATE OR ALTER VIEW [VWSDEST].[V_DIFFERENCE_RESULTS_PER_REGION__HOSPITAL_TOTAL_COUNTS_PER_REGION] AS
...
SELECT 
    ...
    LAG(DBO.NO_NEGATIVE_NUMBER_F(HOSPITAL_TOTAL_COUNTS_PER_REGION), 1, NULL) OVER (PARTITION BY [VRCODE] ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [OLD_VALUE],
	DBO.NO_NEGATIVE_NUMBER_F(HOSPITAL_TOTAL_COUNTS_PER_REGION) -
		LAG(DBO.NO_NEGATIVE_NUMBER_F(HOSPITAL_TOTAL_COUNTS_PER_REGION), 1, NULL) OVER (PARTITION BY [VRCODE] ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [DIFFERENCE]
