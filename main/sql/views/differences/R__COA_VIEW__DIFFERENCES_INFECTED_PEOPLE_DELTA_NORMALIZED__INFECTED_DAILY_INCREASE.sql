...
WITH LAST_DATE_OF_REPORT AS (
	...
	WHERE DATE_OF_REPORT >=  '2020-03-16 00:00:00.000'
		AND DATE_LAST_INSERTED = (SELECT MAX(DATE_LAST_INSERTED) FROM VWSDEST.POSITIVE_TESTED_PEOPLE)
),
...
SELECT 
	...
	LAG(DBO.NO_NEGATIVE_NUMBER_F([INFECTED_DAILY_INCREASE]), 1, NULL) OVER (ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [OLD_VALUE],
	DBO.NO_NEGATIVE_NUMBER_F([INFECTED_DAILY_INCREASE]) -
		LAG(DBO.NO_NEGATIVE_NUMBER_F([INFECTED_DAILY_INCREASE]), 1, NULL) OVER (ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [DIFFERENCE]
