...
SELECT 
    ...
    LAG(DBO.NO_NEGATIVE_NUMBER_F(INFECTED_INCREASE_PER_REGION), 1, NULL) OVER (PARTITION BY [VRCODE] ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [OLD_VALUE],
	DBO.NO_NEGATIVE_NUMBER_F(INFECTED_INCREASE_PER_REGION) -
		LAG(DBO.NO_NEGATIVE_NUMBER_F(INFECTED_INCREASE_PER_REGION), 1, NULL) OVER (PARTITION BY [VRCODE] ORDER BY [DATE_LAST_INSERTED], [DATE_OF_REPORT_UNIX]) AS [DIFFERENCE]
