...
/****** Object:  View [VWSDEST].[V_DIFFERENCES_DISABILITY__INFECTED_LOCATIONS_TOTAL_NL]    Script Date: 18-01-2021 ******/
CREATE OR ALTER  VIEW [VWSDEST].[V_DIFFERENCES_DISABILITY__INFECTED_LOCATIONS_TOTAL_NL] AS
...
SELECT 
	...
	LAG(DBO.NO_NEGATIVE_NUMBER_I([TOTAL_INFECTED_LOCATIONS]), 1, NULL) OVER (ORDER BY DATE_LAST_INSERTED,[DATE_OF_REPORT_UNIX]) AS [OLD_VALUE],
	DBO.NO_NEGATIVE_NUMBER_I([TOTAL_INFECTED_LOCATIONS]) -
		LAG(DBO.NO_NEGATIVE_NUMBER_I([TOTAL_INFECTED_LOCATIONS]), 1, NULL) OVER (ORDER BY DATE_LAST_INSERTED,[DATE_OF_REPORT_UNIX]) AS [DIFFERENCE]
FROM VWSDEST.DISABILITY
WHERE DATE_OF_REPORT >=  '2020-02-27 00:00:00.000' --In line with disability views
