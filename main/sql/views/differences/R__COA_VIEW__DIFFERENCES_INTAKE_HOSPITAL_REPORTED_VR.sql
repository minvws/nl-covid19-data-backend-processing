...
SELECT 
    ...
    LAG(DBO.NO_NEGATIVE_NUMBER_F(REPORTED), 1, NULL) OVER (PARTITION BY [VR_CODE] ORDER BY [DATE_LAST_INSERTED], [DATE_OF_STATISTICS]) AS [OLD_VALUE],
	DBO.NO_NEGATIVE_NUMBER_F(REPORTED) -
		LAG(DBO.NO_NEGATIVE_NUMBER_F(REPORTED), 1, NULL) OVER (PARTITION BY [VR_CODE] ORDER BY [DATE_LAST_INSERTED], [DATE_OF_STATISTICS]) AS [DIFFERENCE]
