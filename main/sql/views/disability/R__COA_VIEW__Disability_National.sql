...
CREATE OR ALTER VIEW VWSDEST.V_DISABILITY_NATIONAL
AS
SELECT
...
AND DATE_OF_REPORT <  (SELECT MAX(DATE_OF_REPORT) FROM VWSDEST.DISABILITY)
AND DATE_LAST_INSERTED = (SELECT max(DATE_LAST_INSERTED) FROM VWSDEST.DISABILITY)
