...
CREATE OR ALTER VIEW VWSDEST.V_GGD_TESTED_PEOPLE_AVERAGE_NL AS
SELECT 
       [NEW_DATE_OF_REPORT_UNIX]                                                                                                                                  AS DATE_END_UNIX
      ,CASE WHEN [DATE_RANGE_START_UNIX] IS NULL THEN (SELECT MIN(DATE_RANGE_START_UNIX) FROM [VWSDEST].[GGD_TESTED_PEOPLE_NL]) ELSE DATE_RANGE_START_UNIX END    AS DATE_START_UNIX
