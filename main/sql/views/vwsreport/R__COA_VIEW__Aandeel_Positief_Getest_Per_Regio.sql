...
select  distinct a.*
from THIRD_CTE a
where date_of_report = (select max(DATE_OF_REPORT) from THIRD_CTE)) a
...
from THIRD_CTE a
join [VWSSTATIC].[SAFETY_REGIONS_PER_MUNICIPAL] b
on a.VRCODE = b.VRCODE
where date_of_report = (select max(DATE_OF_REPORT) from THIRD_CTE)) a, FOURTH_CTE b
