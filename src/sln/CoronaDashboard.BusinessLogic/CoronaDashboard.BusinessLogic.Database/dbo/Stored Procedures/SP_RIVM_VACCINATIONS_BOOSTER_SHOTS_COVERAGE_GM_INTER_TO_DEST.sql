-- COPYRIGHT (C) 2020 DE STAAT DER NEDERLANDEN, MINISTERIE VAN VOLKSGEZONDHEID, WELZIJN EN SPORT.
 ...
 CREATE   PROCEDURE [DBO].[SP_RIVM_VACCINATIONS_BOOSTER_SHOTS_COVERAGE_GM_INTER_TO_DEST]
 AS
 BEGIN
     WITH CTE AS (
         SELECT 
         ...
         FROM [VWSINTER].[RIVM_VACCINATIONS_BOOSTER_SHOTS_COVERAGE_GM] coverage
         INNER JOIN [VWSSTATIC].[COHORT_TO_AGE_GROUPS_MAPPING] age_groups 
             ON age_groups.[COHORT] = coverage.[BIRTH_YEAR]
         WHERE [DATE_LAST_INSERTED] = (SELECT MAX([DATE_LAST_INSERTED]) FROM [VWSINTER].[RIVM_VACCINATIONS_BOOSTER_SHOTS_COVERAGE_GM])
     )
     INSERT INTO [VWSDEST].[RIVM_VACCINATIONS_BOOSTER_SHOTS_COVERAGE_GM] (
             [VERSION],
             [DATE_OF_REPORT],
             [DATE_OF_STATISTICS],
             [POPULATION],
