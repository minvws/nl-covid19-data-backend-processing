-- COPYRIGHT (C) 2020 DE STAAT DER NEDERLANDEN, MINISTERIE VAN VOLKSGEZONDHEID, WELZIJN EN SPORT.
 ...
 CREATE   PROCEDURE [dbo].[SP_BEHAVIOR_NATIONAL]
 AS
 BEGIN
     ...
     DECLARE @posttest_isolation VARCHAR(50) = 'bij_klachten_postest_isolatie';
 --POSTTEST_ISOLATION
     ...
     SELECT
         ...
         (SELECT [VALUE] FROM COMPLIANCE_CTE T1 WHERE TD.DATE_OF_MEASUREMENT=T1.DATE_OF_MEASUREMENT AND LOWER(T1.[INDICATOR]) = @POSTTEST_ISOLATION) AS POSTTEST_ISOLATION_COMPLIANCE,
         (SELECT [CHANGE] FROM COMPLIANCE_CTE T1 WHERE TD.DATE_OF_MEASUREMENT=T1.DATE_OF_MEASUREMENT AND LOWER(T1.[INDICATOR]) = @POSTTEST_ISOLATION) AS POSTTEST_ISOLATION_COMPLIANCE_TREND,
         ...
         (SELECT [VALUE] FROM SUPPORT_CTE T2 WHERE TD.DATE_OF_MEASUREMENT=T2.DATE_OF_MEASUREMENT AND LOWER(T2.[INDICATOR]) = @POSTTEST_ISOLATION) AS POSTTEST_ISOLATION_SUPPORT,
         (SELECT [CHANGE] FROM SUPPORT_CTE T2 WHERE TD.DATE_OF_MEASUREMENT=T2.DATE_OF_MEASUREMENT AND LOWER(T2.[INDICATOR]) = @POSTTEST_ISOLATION) AS POSTTEST_ISOLATION_SUPPORT_TREND
