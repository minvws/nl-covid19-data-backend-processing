-- UNION VIEW
 ...
 CREATE   VIEW [VWSDEST].[V_INTENSIVE_CARE_BED_LCPS] AS
     WITH BASE_CTE AS (
         SELECT * FROM [VWSDEST].[V_INTENSIVE_CARE_BED_LCPS_API_2023] WHERE DATE_UNIX >= dbo.CONVERT_DATETIME_TO_UNIX('2022-12-12')
         UNION
         SELECT * FROM [VWSDEST].[V_IC_BED_OCCUPANCY] WHERE DATE_UNIX < dbo.CONVERT_DATETIME_TO_UNIX('2022-12-12')
     )
     SELECT
         t1.[DATE_UNIX] AS [DATE_UNIX],
         dbo.CONVERT_DATETIME_TO_UNIX(dateadd(day,-6,dbo.CONVERT_UNIX_TO_DATETIME(t1.[DATE_UNIX]))) AS [DATE_START_UNIX],
         t1.[DATE_UNIX] AS [DATE_END_UNIX],
         t1.[BEDS_OCCUPIED_COVID],
         --t1.[BEDS_OCCUPIED_NON_COVID], -- removed on request of FE(See: COR-1275)
         CAST(AVG(CAST(t2.[BEDS_OCCUPIED_COVID] AS FLOAT)) AS DECIMAL(16,2)) AS [BEDS_OCCUPIED_COVID_MOVING_AVERAGE],
