...
CREATE   VIEW [VWSDEST].[V_DIFFERENCE_INTAKE_INTENSIVECARE_REPORTING_DATE_RIVM] AS
...
SELECT  [ID]
      ...
      ,	DBO.NO_NEGATIVE_NUMBER_F([IC_ADMISSION_NOTIFICATION]) -
		    DBO.NO_NEGATIVE_NUMBER_F(IC_ADMISSION_NOTIFICATION_7D_AVG) AS [DIFFERENCE]
...
SELECT 
     ...
     DATE_OF_STATISTICS											                  AS DATE_OF_REPORT
	  ...
	  ,CAST(ROUND(IC_ADMISSION_NOTIFICATION_7D_AVG, 0) as int)  AS OLD_VALUE
	  ,DBO.NO_NEGATIVE_NUMBER_F([IC_ADMISSION_NOTIFICATION]) -
		CAST(ROUND(IC_ADMISSION_NOTIFICATION_7D_AVG, 0) as int)		AS [DIFFERENCE]
FROM BASE_CTE T1
