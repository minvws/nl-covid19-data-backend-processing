CREATE   VIEW [VWSDEST].[V_DIFFERENCE_NURSING_HOME__INFECTED_LOCATIONS_TOTAL_PER_REGION] AS
...
WITH LAST_DATE_OF_REPORT AS (
	...
	WHERE DATE_OF_REPORT >=  '2020-03-16 00:00:00.000'
		AND DATE_OF_REPORT <  (SELECT MAX(DATE_OF_REPORT) FROM VWSDEST.NURSING_HOMES_PER_REGION WITH(NOLOCK))
		AND DATE_LAST_INSERTED = (SELECT MAX(DATE_LAST_INSERTED) FROM VWSDEST.NURSING_HOMES_PER_REGION WITH(NOLOCK))
	GROUP BY VRCODE
),
