...
CREATE   VIEW [VWSDEST].[V_DIFFERENCE_POSITIVE_TESTED_PEOPLE__INFECTED_DAILY_TOTAL] AS

SELECT DATE_OF_REPORT
	  ,NEW_DATE_OF_REPORT_UNIX AS NEW_DATE_UNIX
	  ,OLD_DATE_OF_REPORT_UNIX AS OLD_DATE_UNIX
	  ,T1.GMCODE
	  ,CAST(CASE WHEN OLD_VALUE IS NULL THEN 0 ELSE OLD_VALUE END AS INT) AS OLD_VALUE
	  ,CAST(CASE WHEN [DIFFERENCE] IS NULL THEN 0 ELSE [DIFFERENCE] END AS INT) AS [DIFFERENCE]
