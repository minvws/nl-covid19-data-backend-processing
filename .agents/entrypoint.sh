...
if [ -z "$DEVOPS_SERVER_INSTANCE_URL" ]; then
    echo 1>&2 "error: missing DEVOPS_SERVER_INSTANCE_URL environment variable"
    exit 1
fi
...
DEVOPS_AGENT_AGENT_PACKAGES=$(curl -LsS \
    ...
    "$DEVOPS_SERVER_INSTANCE_URL/_apis/distributedtask/packages/agent?platform=$TARGETARCH&top=1")
...
if [ -z "$DEVOPS_AGENT_AGENT_PACKAGE_LATEST_URL" -o "$DEVOPS_AGENT_AGENT_PACKAGE_LATEST_URL" == "null" ]; then
    ...
    echo 1>&2 "check that account '$DEVOPS_SERVER_INSTANCE_URL' is correct and the token is valid for that account"
    exit 1
fi
...
./config.sh --unattended \
    ...
    --url "$DEVOPS_SERVER_INSTANCE_URL" \
    --auth PAT \
